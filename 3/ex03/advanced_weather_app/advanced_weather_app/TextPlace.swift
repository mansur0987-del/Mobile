		...
		VStack {
			if CheckerIsErrors(location: location, locationManager: locationManager) != "OK" {
				...
				if IdActiveButton == 0, location.current != nil {
					...
					DailyView(daily : $location.daily, IdActiveButton: $IdActiveButton, isPortait: isPortait)
				}
				else if IdActiveButton == 2, location.week.count > 0 {
					WeekView(week: $location.week, IdActiveButton: $IdActiveButton, isPortait: isPortait)
				}
			}
		}
		.font(.title3)
